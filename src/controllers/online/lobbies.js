...
router.post('/:paddedID/lobby/:lobbyNumber/player/:playerID', (req, res, next) => {
  ...
  const paddedID = parseInt(req.params.paddedID)
  const lobbyNumber = parseInt(req.params.lobbyNumber)
  const playerID = req.params.playerID
  const realID = Math.floor(paddedID / parseInt(process.env.SECRET_ID_MULTIPLIER))
  ...
  User.findOne({ where: { id: realID } }).then(result => {
    ...
    Lobby.findOne(
      ...
      let currentData = lobbyResult.dataValues.current_data || []
      /*
